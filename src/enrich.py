...
def account_model():
    ...
    # renaming cols for clarification
...
def clean_transaction():
    # slowest step, inefficient
    trans_df = pd.read_parquet(f"{SOURCE_PATH}trans.parquet") #read in DF
    trans_df['prev_balance'] = trans_df.apply(lambda x: x['balance'] + x['amount'] if x['type'] == 'debit' 
        else x['balance'] - x['amount'], axis=1) # calculate balance before transaction 
    trans_df['prev_balance'] = trans_df['prev_balance'].round(1)
    trans_df = trans_df.sort_values(['account_id','date'])
    # logic to identify last transaction of each day and flag it
